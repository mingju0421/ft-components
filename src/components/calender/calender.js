...
let day = ({ year, month }, length) => {
  let result = [];
  let start = moment(`${year}-${month}-01`);
  // 如果当前为周末, dow为0, 赋值为7
  let dow = start.day() || 7;
  // start设置为周一
  start.add(1 - dow, "d");
...
let difference = {
  ...
  2: () => {
    let weekOfday = moment().day(); // 计算今天是这周第几天  周日为一周中的第一天
  ...
  7: () => {
    let start_time = moment().startDay("Q");
    let end_time = moment().endDay("Q");
    return {
      start_time,
      end_time,
      date_trunc: "month",
