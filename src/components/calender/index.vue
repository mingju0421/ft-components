			...
			>
						...
						<div class="DAY">
							<div class="tabHeader">
								...
								<i
									...
									:class="{disabled: control.day.year <= 2016 && control.day.month <= 1}"
									@click="subtractMonth('day')"
								></i>
								{{control.day.year}}年{{control.day.month}}月
								<i
									class="el-icon-arrow-right pointer"
									:class="{disabled: control.day.year >= new Date().getFullYear() && control.day.month >= new Date().getMonth() + 1}"
									@click="addMonth('day')"
								></i>
								<i
									class="el-icon-d-arrow-right pointer"
									:class="{disabled: control.day.year >= new Date().getFullYear()}"
									@click="addYear('day')"
								></i>
							</div>
							<div class="tabContent">
								<div class="week">
									<span v-for="(item, index) in week" class="weekItem" :key="index">{{item}}</span>
								</div>
								<div
									v-for="(item, index) in day"
									:class="[{weekHover: activeName == 'WEEK' && showDateTrunc}, {selected: control.selected == JSON.stringify(item) && showDateTrunc}]"
									class="week pointer"
									@click="() => {activeName == 'WEEK' && showDateTrunc && selectWeek(item)}"
									:key="index"
								>
									<span
										v-for="(oneDay, i) in item"
										:data-week="i > 2 ? `${oneDay.year}-${+oneDay.week + 1}` : `${oneDay.year}-${oneDay.week}`"
										@click="selectDay(oneDay, i)"
										class="weekItem item"
										:class="[{disabled: new Date(oneDay.year, oneDay.month - 1, oneDay.day) > new Date()},{noEvents: activeName == 'WEEK' && showDateTrunc},{selected: control.selected == `${oneDay.year}-${oneDay.month}-${oneDay.day}`}, {ontherMonth: oneDay.month != control.day.month}, {selectedDay: !showDateTrunc &&  (i > 2 ? `${oneDay.year}-${+oneDay.week + 1}` : `${oneDay.year}-${oneDay.week}`) == selectWeekDay}]"
										:key="i"
									>{{oneDay.day}}</span>
								</div>
							</div>
					...
					<div class="detaRight" v-if="showDateTrunc">
							...
							<div class="tabContent">
								...
								>
									<span
										...
										@click="selectDay(oneDay, i)"
										class="weekItem item"
										:class="[{disabled: new Date(oneDay.year, oneDay.month - 1, oneDay.day) > new Date()},{noEvents: activeName == 'WEEK' && showDateTrunc},{selected: control.selected == `${oneDay.year}-${oneDay.month}-${oneDay.day}`}, {ontherMonth: oneDay.month != control.day.month}]"
										:key="i"
									>{{oneDay.day}}</span>
								</div>
							</div>
						...
						<div class="MONTH" v-else-if="activeName == 'MONTH'">
							<div class="tabHeader">
								...
								<i
									class="el-icon-d-arrow-right pointer"
									:class="{disabled: control.month.year >= new Date().getFullYear()}"
									@click="addYear('month')"
								></i>
							</div>
							<div class="tabContent">
								<div
									...
									:class="[{selected: control.month.year == control.month.selectedYear &&  control.selected == item}, {disabled: new Date() < new Date(`${control.month.year}-${index > 8 ? index + 1: '0' + (index + 1)}`)}]"
									:key="index"
								>{{item}}</div>
							</div>
						</div>
						...
						<div class="SEASON" v-else-if="activeName == 'SEASON'">
							<div class="tabHeader">
								...
								<i
									...
									:class="{disabled: control.season.year >= new Date().getFullYear()}"
							...
							<div class="tabContent">
								<div
									...
									:key="index"
									:class="[{selected: control.season.year == control.season.selectedYear && control.selected == item.label}, {disabled: new Date() < new Date(control.season.year + item.time)}]"
						...
						<div class="YEAR" v-else-if="activeName == 'YEAR'">
							<div class="tabHeader">2016年 - {{new Date().getFullYear()}}年</div>
							<div class="tabContent">
								<div
									class="year"
				...
				<div class="selectItem" slot="reference">
					...
					<div
						...
						class="showText"
					>{{control.selectedType || control.selected}}</div>
					<div v-else>{{date.start_time}} 至 {{date.date_sign === 'all' ? new $moment().format('YYYY-MM-DD') : date.end_time}}</div>
				</div>
			</el-popover>
		</div>
	...
	methods: {
		...
		selectDay(dayObj = {}, i) {
			...
			if (this.showDateTrunc) {
				...
				let date = new Moment(start_time).day();
				let start, end;
				if (date > 3) {
					start = new Moment(start_time).day(4).format();
					end = new Moment(start_time).day(10).format();
				} else {
					start = new Moment(start_time).day(-3).format();
					end = new Moment(start_time).day(3).format();
				}
				this.selectWeekDay =
					i > 2 ? `${dayObj.year}-${+dayObj.week + 1}` : `${dayObj.year}-${dayObj.week}`;
				...
				let start, end;
				let time = new Date();
				start = new Moment(time).day(-3).format();
				end = new Moment(time).day(3).format();
				this.selectWeekDay = new Moment(time).day(3).format();
				this.date = {
					...this.date,
					end_time: end,
					start_time: start,
		...
		selectMonth(num) {
			...
			let end_time = new Moment(start_time).endDay('M')
		...
		selectSeason(num) {
			...
			let end_time = new Moment(start_time).endDay('Q');
			...
			this.date = { ...this.date, ...obj };
			this.control.selectedType = `${this.control.season.year}年第${num + 1}季度`;
			this.closePopover();
		},
		/** 选择年 */
		selectYear(year) {
			...
			let end_time = new Moment(start_time).endDay('Y');
